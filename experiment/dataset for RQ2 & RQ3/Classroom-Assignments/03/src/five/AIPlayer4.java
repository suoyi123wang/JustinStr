...
public class AIPlayer4 extends Player {
	private String[][] A;
	byte x, y;
	String[] pattern;

	public AIPlayer4(int color, String player) {
		this.color = color;
		this.player = player;

	}

	private void var(int color) {
		A = new String[15][15];
		for (int i = 0; i < 15; i++) {
			for (int j = 0; j < 15; j++) {
				if (color == 0) {
					if (Board.board[0][i + 1][j + 1]) {
						if (Board.board[1][i + 1][j + 1])
							A[i][j] = "x";
						else if (Board.board[2][i + 1][j + 1])
							A[i][j] = "y";
					} else
						A[i][j] = "0";
				} else if (color == 1) {
	...
	private double evaluate(int x, int y, boolean defence) {
		...
		for (int j = 0; j < 4; j++) {
			...
			if (defence) {
				pattern[j] = pattern[j].replace('x', 'z').replace('y', 'x')
						.replace('z', 'y');
			}
		}
		// if (d) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (pattern[i].matches("(.*)yxxxx0(.*)")
					...
					|| p.matches("(.*)x0xxx(.*)") || p.matches("(.*)xxx0x(.*)")
					|| p.matches("(.*)xx0xx(.*)")) {
				p += "a";
				continue;
			}
			...
			if (pattern[i].matches("(.*)yxxx0(.*)")
					...
					|| p.matches("(.*)x0xx0(.*)") || p.matches("(.*)0xx0x(.*)")
					|| p.matches("(.*)xx0x0(.*)") || p.matches("(.*)0x0xx(.*)")) {
				p += "b";
				continue;
			}
			...
			if (pattern[i].matches("(.*)xx000(.*)")
					...
					|| p.matches("(.*)x0x00(.*)") || p.matches("(.*)00x0x(.*)")) {
				p += "c";
				continue;
			}
